...
export default function ClientProfile({ params }: { params: { email: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/user/${encodeURIComponent(params.email)}`);
  ...
  const handleConnect = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/connections/send`, {
        ...
        body: JSON.stringify({ targetEmail: params.email })
      });
